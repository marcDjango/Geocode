...
class ReservaManager extends AbstractManager {
  ...
  async readAllReservation() {
    ...
    const [rows] = await this.database.query(
      `select id, user_id, charging_station_id,DATE_FORMAT(reservation_date, '%Y-%m-%d') AS reservation_date, amount_paid from ${this.table}`
    );
    // Returning all rows
    return rows;
  }
  ...
  async readReservation(id) {
    ...
    const [row] = await this.database.query(
      `select id, user_id, charging_station_id,DATE_FORMAT(reservation_date, '%Y-%m-%d') AS reservation_date, amount_paid from ${this.table} where id = ?`,
      [id]
    );
