...
class UserManager extends AbstractManager {
  ...
  async readAllUser() {
    ...
    const [rows] = await this.database.query(
      `select id , name, firstname, email, gender, DATE_FORMAT(date_of_birth, '%Y-%m-%d') AS date_of_birth , postal_code, city, number_vehicles, hashed_password, profil_image, role from ${this.table}`
    );
    // Returning all rows
    return rows;
  }
  ...
  async readUser(id) {
    ...
    const [row] = await this.database.query(
      `select id , name, firstname, email, gender, DATE_FORMAT(date_of_birth, '%Y-%m-%d') AS date_of_birth , postal_code, city, number_vehicles, hashed_password, profil_image, role from ${this.table} where id = ?`,
      [id]
    );
